...
class DoubleBatchedGemv(BaseBLAS, Op):
    ...
    def perform(self, node, inputs, outstor):
        x = inputs[0]
        assert 0 == (len(inputs) - 1) % 4
        assert (len(inputs) - 1) / 4 == self.N
        ii = 1
        y = np.zeros_like(x)
        while ii + 4 <= len(inputs):
            ust = inputs[ii + 0]
            vst = inputs[ii + 1]
            U = inputs[ii + 2]
            V = inputs[ii + 3]
            xi = x[ust:ust + len(U)]
            yi = y[vst:vst + len(V)]
            #print x.shape, vst, ust, U.shape, V.shape
            #print xi.shape, yi.shape
            #print np.dot(xi, U).shape
            yi += np.dot(np.dot(xi, U), V.T)
            ii += 4
        outstor[0][0] = y
